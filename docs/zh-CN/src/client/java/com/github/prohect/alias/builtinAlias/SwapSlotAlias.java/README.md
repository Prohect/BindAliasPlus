...
_无公开/受保护字段（内部使用私有 `SlotRef` 记录表示槽位）。_
...
| [resolveSlot](resolveSlot.md) | `resolveSlot(menu, SlotRef)` | 在菜单中查找匹配 SlotRef 的 Slot 对象 |
| [swapButton](swapButton.md) | `swapButton(Slot)` | 若快捷栏/副手可寻址则获取 SWAP 点击按钮 |
| [swapInMenu](swapInMenu.md) | `swapInMenu(...)` | 使用 SWAP 或 PICKUP 策略交换两个槽位 |
| [clickSlot](clickSlot.md) | `clickSlot(im, menu, slot, btn, act, player)` | 通过 `ClientPlayerInteractionManager#clickSlot` 执行单次容器点击 |
| [swapSlotOffhand](swapSlotOffhand.md) | `swapSlotOffhand(net, idx)` | 基于数据包的快捷栏↔副手交换 |
