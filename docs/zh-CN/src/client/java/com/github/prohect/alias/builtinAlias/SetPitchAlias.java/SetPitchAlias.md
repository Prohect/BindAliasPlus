...
将玩家相机俯仰角设置为绝对角度的内置别名。继承 `BuiltinAliasWithDoubleArgs`。
...
**别名名称：** `setPitch` — 用法：`setPitch\deg`，其中 `deg` 是以度为单位的绝对俯仰角。

**行为：** 将玩家的俯仰角设置为参数指定的精确值。与应用相对旋转（`current + delta`）的 `PitchAlias` 不同，此别名是绝对的。

**有效范围：** Minecraft 强制俯仰角在 [-90, 90] 范围内。负值向上看，正值向下看。超出此范围的值由游戏引擎钳制。

**参数解析：** 参数通过 `VarAlias.resolveDouble()` 解析，支持字面数字和变量名。

**要求：** `mc.player` 必须非 null。如果玩家为 null，则记录一条警告。

**与 PitchAlias 的区别：** `setPitch` 是绝对的（设置为特定角度），`pitch` 是相对的（在当前角度上加减）。
...
| 条目 | 说明 |
|------|-------------|
| [PitchAlias](../PitchAlias.java/PitchAlias.md) | 相对俯仰角旋转 |
