...
触发原版选取方块/实体行为的一次性别名——将玩家正在注视的方块或实体选取到选中的快捷栏槽位。继承 `BuiltinAliasWithoutArgs`。
...
**别名名称：** `pickItem`（内部，作为 `pickItem` 暴露）。

**行为：** 模拟按下原版选取方块按键绑定（`pickItemKey`，默认：鼠标中键）。将按键绑定设置为"按下"并递增 `timesPressed`，使游戏在下一个轮询周期调用 `pickBlockOrEntity()`。这会选中与目标方块/实体匹配的快捷栏槽位，或将物品栏中匹配的物品移动到选中的槽位。

**界面抑制：** 当 `Alias.isUnderTextInputScreen()` 返回 true 时，别名被取消。

**要求：** `mc.player` 必须非空（否则静默返回）。

**与原版选取方块的关键区别：**
- 在创造模式中，方块/实体直接给予选中的槽位。
- 在生存模式中，如果物品栏中有与目标方块/实体匹配的物品，则将其（通过 SWAP）从物品栏移动到选中的槽位。
...
| [SlotAlias](../SlotAlias.java/SlotAlias.md) | 直接选择快捷栏槽位（不含选取方块逻辑） |
