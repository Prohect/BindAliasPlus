...
打开或关闭玩家物品栏界面的内置开关别名（`+openInventory` / `-openInventory`）。继承自 `BuiltinAliasWithBooleanArgs`。
...
**行为：**
- `+openInventory`（flag=1）：通过调用 `sendOpenInventory()`（服务器数据包）和 `McScreenHelper.setScreen()`（传入新的 `InventoryScreen`）打开玩家物品栏界面。文本输入界面打开时被屏蔽。任何界面已打开时被屏蔽。
- `-openInventory`（flag=0）：仅当当前界面是容器界面（`isInContainerScreen()`）时关闭它。若没有容器界面打开，则为空操作。

**界面抑制：** 当 `Alias.isUnderTextInputScreen()` 返回 true 时，按下事件（`+openInventory`）被取消。松开事件从不被抑制。

**与 ToggleInventoryAlias 的区别：** 此别名通过 +/- 模式提供独立的打开/关闭控制，而 `toggleInventory` 每次调用在打开与关闭之间切换。
