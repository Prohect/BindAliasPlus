...
字符串参数别名，用于将已解锁的配方放入合成格。不进行任何合成——取出产物是单独的操作。错误报告到本地聊天；成功则记录日志。
...
| 名称 | 签名 | 说明 |
|------|-----------|-------------|
| [run](run.md) | `ApplyRecipeAlias run(String args)` | 按查询查找配方并将其放入打开的配方菜单 |
| [chatError](chatError.md) | `static void chatError(ClientPlayerEntity, String)` | 向玩家的本地聊天发送错误消息 |
