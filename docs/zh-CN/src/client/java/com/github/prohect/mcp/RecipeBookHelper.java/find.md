...
第一个匹配的 `RecipeInfo`；没有已解锁配方匹配时为 `null`。

## 备注

解析顺序：首先尝试精确注册表 ID 匹配（带或不带 `"minecraft:"` 前缀）。若无匹配，则回退到对语言显示名的不区分大小写子串搜索。内部调用 `unlocked(mc)`，因此调用之间不假定有缓存。
