...
| 名称 | 类型 | 说明 |
|------|------|-------------|
| `args` | `String` | 别名名称和按键，用 `\` 分隔：`aliasName\<key>` |

## 备注

1. 构造命令行：`"bind" + Alias.divider4AliasDefinition + normalizedArgs`。
2. 规范化参数：将参数分隔符（`\`）的每次出现替换为正确的别名定义分隔符。这确保了分隔符格式一致。
3. 检查 `MinecraftClient.getInstance().player` 是否非空。如果为空，记录警告。
4. 通过 `player.connection.sendCommand(line)` 发送命令。

服务器处理 `/bind` 命令以创建持久按键绑定，将物理按键或鼠标按钮与别名名称关联起来。
