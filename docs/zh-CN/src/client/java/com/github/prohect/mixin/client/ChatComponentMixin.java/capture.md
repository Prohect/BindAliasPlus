...
由 `captureMessage` 调用的私有静态辅助方法。通过 `GameChannels.post(CHAT, text)` 将提取的消息文本发布到 [`GameChannels.CHAT`](../../../mcp/GameChannels.java/CHAT.md)。不过滤或转换文本——原始的 `Text#getString()` 输出原样转发。线程安全，因为 `GameChannels.post` 内部同步。
