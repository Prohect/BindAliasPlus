...
模拟按住/松开玩家列表按键（Tab）的开关别名（`+playerList` / `-playerList`）。按住时显示在线玩家浮层。继承自 `BuiltinAliasWithBooleanArgs`。
...
**行为：**
- `+playerList`（flag=1）：按下原版 `keyPlayerList` 按键绑定（Tab），使在线玩家浮层可见，并递增 `clickCount` 以产生首次按下的刻行为。
- `-playerList`（flag=0）：松开 `keyPlayerList` 按键绑定。

**用例：** 主要供 agent/MCP 截取玩家列表浮层的截图，以识别服务器上有哪些玩家在线。

**界面抑制：** 当 `Alias.isUnderTextInputScreen()` 返回 true 时，按下事件（`+playerList`）被取消。松开事件从不被抑制。

**重新应用行为：** 继承 `BuiltinAliasWithBooleanArgs` 的 `reapplyToGameKeyMapping()`——界面切换后，若 `flag` 为 true，按键会重新应用到游戏的按键映射。在 `ReapplyAlias.SUPPORTED_ACTIONS` 中列为 `"playerList"`。
