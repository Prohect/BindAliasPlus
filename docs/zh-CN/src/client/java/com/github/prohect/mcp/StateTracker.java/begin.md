...
| 名称 | 类型 | 说明 |
|------|------|-------------|
| `full` | `boolean` | `true` 强制包含所有状态成员（用于 `/state`），`false` 仅返回变更成员的 diff |
...
必须在 Minecraft 主线程上调用（访问 `Minecraft.getInstance()`）。通过 `synchronized` 保证线程安全。

1. **世界变更检测**：若 `BindAliasClient.joinTick` 与存储的基线不同，则强制 `full = true` 并重置之前的快照。
2. **状态收集**：调用 `GameStateCollector.collect()` 获取当前快照。
3. **成员级 diff**：对每个当前状态成员，若 `full` 或值与之前快照不同则包含。`held_keys` 非空时强制包含。
4. **消失的成员**：之前快照中存在但当前快照中缺失的任何键序列化为 `null`。
5. **容器 diff**：调用 `containerSnapshot()`，与 `lastContainer` 比较。`full` / 打开时 / 菜单身份变更时输出完整视图；否则输出槽位级 diff。
6. **快捷栏 diff**：同样模式 —— `full` / 世界变更时输出完整视图；否则按槽位 diff。
7. **基线更新**：将当前快照存储为 `last`。
