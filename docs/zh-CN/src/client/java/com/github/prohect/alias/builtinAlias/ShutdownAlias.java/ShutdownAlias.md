...
干净地关闭游戏的一次性别名。继承 `BuiltinAliasWithoutArgs`。
...
**别名名称：** `builtinShutdown`（内部，作为 `builtinShutdown` 暴露）。

**行为：** 记录关闭消息并调用 `MinecraftClient.getInstance().scheduleStop()` 安排优雅的游戏停止。游戏在当前刻完成后关闭。（Yarn：`scheduleStop()`；Mojang：`stop()`）

**使用场景：** 专为使用 CFG 自动加载功能的自动化测试工作流设计——在配置中定义测试别名，运行它们，然后在最后调用 `builtinShutdown` 干净退出。也适用于 MCP agent 终止游戏会话。

**无界面抑制：** 在任意界面上都有效（这是系统操作，不是游戏输入）。
