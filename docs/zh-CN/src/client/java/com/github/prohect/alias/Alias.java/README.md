...
别名执行引擎的核心接口。声明了全局注册表、解析工具、界面守卫以及 `run` 契约。

## 字段

| 名称 | 类型 | 说明 |
|------|------|-------------|
| [aliasesWithArgs](aliasesWithArgs.md) | `HashMap<String, AliasWithArgs<?>>` | 接受参数的推荐内置别名 |
| [aliasesWithArgs_notSuggested](aliasesWithArgs_notSuggested.md) | `HashMap<String, AliasWithArgs<?>>` | 接受参数的内部 / 非推荐别名 |
| [aliasesWithoutArgs](aliasesWithoutArgs.md) | `HashMap<String, AliasWithoutArgs<?>>` | 不接受参数的推荐别名（内置 + 用户） |
| [aliasesWithoutArgs_notSuggested](aliasesWithoutArgs_notSuggested.md) | `HashMap<String, AliasWithoutArgs<?>>` | 不带参数的内部 / 非推荐别名 |
| [aliasesWithoutArgs_fromBindCommand](aliasesWithoutArgs_fromBindCommand.md) | `HashMap<String, AliasWithoutArgs<?>>` | 由 `bind` 命令创建的、用于按键绑定查找的别名 |
...
| 名称 | 签名 | 说明 |
|------|-----------|-------------|
| [run](run.md) | `T run(String args)` | 核心契约——用给定参数执行别名；返回 `this` 以便链式调用 |
...
| [isInContainerScreen](isInContainerScreen.md) | `static boolean isInContainerScreen()` | `HandledScreen` 是否打开 |
...
| [AliasWithoutArgs](AliasWithoutArgs.java/AliasWithoutArgs.md) | 不带参数别名的子接口 |
| [UserAlias](UserAlias.java/UserAlias.md) | 主要调用方——解析链并通过这些注册表分发到别名 |
| [BindAliasClient](BindAliasClient.java/BindAliasClient.md) | 将别名注册进这些注册表的地方 |
