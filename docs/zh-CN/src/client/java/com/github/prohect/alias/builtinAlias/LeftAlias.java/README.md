...
原版左侧移键（leftKey / A 键）的开关别名。继承 `BuiltinAliasWithBooleanArgs` 的 `+name`/`-name` 模式。

## 字段

| 名称 | 类型 | 说明 |
|------|------|-------------|
| _（除 `BuiltinAliasWithBooleanArgs.flag` 外无其他）_ | `boolean` | 继承：`+left` 为 true，`-left` 为 false |

## 方法

| 名称 | 签名 | 说明 |
|------|-----------|-------------|
| [run](run.md) | `LeftAlias run(String args)` | 按下或松开 `options.leftKey`；仅在文本输入界面上抑制按下 |
