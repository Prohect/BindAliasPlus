...
1. 如果 `MinecraftClient.getInstance().player` 为 null（不在世界中），则立即返回——没有活跃的游戏会话就无法显示消息。
2. 调用 `MinecraftClient.getInstance().inGameHud.getChatHud().addMessage(Text.literal(args))`（Yarn：`Text`；Mojang：`Component`）将文本显示为客户端侧系统消息。

消息出现在玩家的聊天浮层和聊天历史中，但永远不会发送到服务器。只有本地玩家可见。
