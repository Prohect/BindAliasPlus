...
若容器界面打开则返回 `ContainerSnapshot`；若无容器界面打开或玩家为 null 则返回 `null`。

## 备注

提取打开的容器菜单（`AbstractContainerScreen`）的状态。`ContainerSnapshot` 记录捕获：

- `menuIdentity` —— 由容器的槽位数、标题和类型派生的哈希式标识，`StateTracker` 用它检测菜单变化（变化会触发完整状态转储）
- `items` —— 槽位索引→物品描述的映射（仅非空槽位）
- `emptyInv` —— 容器中玩家物品栏部分的压缩空槽位区间
- `grid` —— 合成格表示（2×2 玩家、3×3 工作台、熔炉/燃料进度）

物品描述使用 `appendTooltipIfValuable`，格子格式化使用 `gridJson`。
