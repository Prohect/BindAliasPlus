...
1. **Window guard**: if `window != MinecraftClient.getInstance().getWindow().handle()`, returns immediately.
2. **Key resolution**: creates an `InputUtil.Key` from the event's key code (`Type.KEYSYM.getOrCreate(event.key())`).
