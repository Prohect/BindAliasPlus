...
3. Triggers the vanilla screenshot via `Minecraft.getInstance().options.takeScreenshot()` (which calls `NativeImage#writeToFile` → `NativeImageMixin#onWriteToFile` → completes the future).
...
| [NativeImageMixin.onWriteToFile](../../mixin/client/NativeImageMixin.java/onWriteToFile.md) | The mixin that captures PNG bytes |
