...
The access widener (`bind-alias.accesswidener`) removes the `private` flag from `writeToChannel`, allowing Mixin to generate the invoker. The generated method is abstract — Mixin provides the implementation at compile time that delegates to the original.
