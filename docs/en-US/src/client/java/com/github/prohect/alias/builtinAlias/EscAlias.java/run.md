...
2. If any screen is currently open (`Alias.isUnderAnyScreen()`), calls `getCurrentScreen().onClose()` to close it, then returns. This path applies regardless of the flag value — closing always takes priority.
