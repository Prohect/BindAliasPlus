...
**Initial drop:** On press (`+drop`), one immediate drop is performed — `slotClicked(…, THROW)` in container screens, or `keyDrop.clickCount++` in the 3D game.
...
**Container screen special behavior:** When a container screen (chest, furnace, inventory) is open, drops target the hovered slot using `slotClicked()` with `ContainerInput.THROW`. Ctrl+click detection (`hasControlDown()`) controls whether the entire stack (button 1) or a single item (button 0) is dropped.

**Reapply:** After screen transitions, `reapplyToGameKeyMapping()` sets `keyDrop.setDown(true)` but does **not** increment `clickCount` — this prevents an extra drop from firing when the cursor is re-locked.
