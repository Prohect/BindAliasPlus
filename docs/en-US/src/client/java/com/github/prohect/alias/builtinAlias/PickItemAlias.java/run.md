...
3. Get the vanilla `pickItemKey` keybinding from `mc.options`.
4. Call `pickKey.setPressed(true)` and `pickKey.timesPressed++` to simulate a pick-block keypress.
...
- The `pickItemKey` keybinding state is modified (set down, click count incremented).
