...
1. Save and set `BindAliasPlusClient.silentMode = true` to suppress per-operation log messages.
...
**Side effects**: Removes entries from `Alias.aliasesWithoutArgs`, `BindAliasPlusClient.BINDING_PLUS`, `Alias.aliasesWithoutArgs_fromBindCommand`, `VarAlias.VARIABLES`, and `VarAlias.AUTOLOADED_VARIABLES`.
