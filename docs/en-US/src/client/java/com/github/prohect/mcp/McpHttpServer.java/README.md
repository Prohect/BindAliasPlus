...
| `decodePercent` | `private static String (String)` | Decodes `%XX` sequences. No `+` → space conversion. |
| `sendJson` | `private static void (HttpExchange, int, String)` | Sends a JSON HTTP response. |
| `jsonEscape` | `private static String (String)` | Escapes a string for JSON (handles `"`, `\\`, `\n`, `\r`, `\t`). |
