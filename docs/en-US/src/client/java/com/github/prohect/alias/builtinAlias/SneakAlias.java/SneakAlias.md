...
**Behavior:**
- `+sneak` (flag=1): Presses the vanilla `keyShift` keybinding (Shift), making the player sneak/crouch. Sneaking prevents falling off edges and reduces player height.
- `-sneak` (flag=0): Releases the `keyShift` keybinding.
...
**Movement injection:** Like other movement aliases, this sets the keybinding state directly via `key.setDown(flag)` which is intercepted by `KeyboardInputMixin` for window-focus-independent movement. The `clickCount` is incremented on the press event so the game registers the initial press-action.
