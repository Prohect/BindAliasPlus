...
5. Call `ScreenshotRecorder.saveScreenshot(mc.runDirectory, mc.getFramebuffer(), msg -> mc.execute(() -> mc.inGameHud.getChatHud().addMessage(msg)))` — triggers the vanilla screenshot pipeline (same as pressing F2). (Yarn: `ScreenshotRecorder.saveScreenshot()`; Mojang: `handleGlobalKeyPress()`)
