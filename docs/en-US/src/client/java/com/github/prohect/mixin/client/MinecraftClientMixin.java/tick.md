...
Injected at `HEAD` of `Minecraft#tick()`. Executes four subsystems in order each client tick:

1. **Screen tracking**: `BindAliasClient.currentScreen = McScreenHelper.getCurrentScreen(Minecraft.getInstance())` — updates the global current-screen reference for alias screen-type checks.
