...
**Behavior:** Sends a `ServerboundPlayerActionPacket` with action `SWAP_ITEM_WITH_OFFHAND` to the server. This swaps the player's main hand held item with the offhand item.
...
**Parameters:** The packet is sent with `BlockPos.ZERO` and `Direction.DOWN` as placeholder values (they're ignored by the server for SWAP_ITEM_WITH_OFFHAND actions).

**Screen suppression:** Cancelled when `Alias.isUnderTextInputScreen()` returns true.

**Requirements:** The network handler (`mc.getConnection()`) must be non-null. Logs a warning if null.
