...
3. Get the vanilla `playerListKey` keybinding.
4. Call `key.setPressed(flag)` to press or release the key.
5. If pressing (flag=true), increment `key.timesPressed` so the game registers the initial press event.

**Side effects:** Shows or hides the online-player overlay. The keybinding state (`isDown`, `timesPressed`) is modified.
