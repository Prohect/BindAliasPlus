...
**Behavior:** Selects the specified hotbar slot by calling `inventory.setSelectedSlot(i - 1)` and sending a `ServerboundSetCarriedItemPacket` to the server to synchronize the change.
...
**Implementation note:** The commented-out code at the top of `run()` shows an earlier approach using `KeyMapping` keybindings to simulate hotbar keypresses. The current implementation uses direct inventory manipulation + server packet for reliability.
