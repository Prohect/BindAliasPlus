# BuiltinAliasWithStringArgs (src/client/java/com/github/prohect/alias/BuiltinAliasWithStringArgs.java)

## Syntax

```java
public abstract class com.github.prohect.alias.BuiltinAliasWithStringArgs<T extends com.github.prohect.alias.BuiltinAliasWithStringArgs<T>> extends com.github.prohect.alias.BuiltinAliasWithArgs<T>
```

## Static Initializer

_None._

## Remarks

## See Also

| Item | Description |
|------|-------------|

*Documented for Commit: [7560eeea61820ae4db1314f3e3132a9576194d5a](https://github.com/Prohect/BindAlias/tree/7560eeea61820ae4db1314f3e3132a9576194d5a)*

