...
3. Retrieves `MinecraftClient.getInstance().options.forwardKey` and calls `setPressed(flag)` plus `timesPressed++` on press.

Movement is applied each tick by `KeyboardInputMixin`, which reads `forwardKey.isDown()`.
