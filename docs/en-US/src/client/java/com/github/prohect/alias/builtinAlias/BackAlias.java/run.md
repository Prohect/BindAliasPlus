...
3. Retrieves `MinecraftClient.getInstance().options.backKey` and calls `setPressed(flag)` plus `timesPressed++` on press.

Movement is read each tick by `KeyboardInputMixin`, which checks `backKey.isDown()` to produce the player's backward movement impulse.
