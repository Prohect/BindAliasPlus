...
Simulates a backward movement key press or release by calling `KeyMapping.setDown(flag)` and incrementing `clickCount` on press.
...
3. Obtains `Minecraft.getInstance().options.keyDown`.
4. Calls `backKey.setDown(flag)` to set the key state.
5. If `flag` is true, increments `backKey.clickCount` to ensure the game registers the click.

Side effects: modifies `keyDown.setDown` and `keyDown.clickCount` on the game's `KeyMapping`.

Callers: dispatched by [UserAlias](../../UserAlias.java/UserAlias.md) when the alias name `"builtinBack"` matches an `AliasRecord`.
...
| Item                                                                                         | Description                               |
| -------------------------------------------------------------------------------------------- | ----------------------------------------- |
| [BackAlias](BackAlias.md)                                                                    | Owning class                              |
