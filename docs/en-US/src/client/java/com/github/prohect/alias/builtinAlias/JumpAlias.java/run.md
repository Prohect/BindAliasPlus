...
Simulates a jump key press or release by calling `KeyMapping.setDown(flag)` and incrementing `clickCount` on press.
...
3. Obtains `Minecraft.getInstance().options.keyJump`.
4. Calls `jumpKey.setDown(flag)` to set the key state.
5. If `flag` is true, increments `jumpKey.clickCount` to ensure the game registers the click.

Side effects: modifies `keyJump.setDown` and `keyJump.clickCount` on the game's `KeyMapping`.

Callers: dispatched by [UserAlias](../../UserAlias.java/UserAlias.md) when the alias name `"builtinJump"` matches an `AliasRecord`.
...
| Item                                                                                         | Description                               |
| -------------------------------------------------------------------------------------------- | ----------------------------------------- |
| [JumpAlias](JumpAlias.md)                                                                    | Owning class                              |
