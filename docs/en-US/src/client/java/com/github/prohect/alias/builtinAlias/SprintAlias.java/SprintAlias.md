...
**Behavior:**
- `+sprint` (flag=1): Presses the vanilla `keySprint` keybinding (Ctrl). When combined with `+forward`, the player sprints.
- `-sprint` (flag=0): Releases the `keySprint` keybinding.
...
**Movement injection:** Sets the keybinding state directly via `key.setDown(flag)` which is intercepted by `KeyboardInputMixin` for window-focus-independent sprinting.
