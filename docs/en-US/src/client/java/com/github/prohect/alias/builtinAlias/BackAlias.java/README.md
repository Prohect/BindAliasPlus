...
Switch alias for the vanilla backward movement key (backKey / S key). Inherits the `+name`/`-name` pattern from `BuiltinAliasWithBooleanArgs`.
...
|------|-----------|-------------|
| [run](run.md) | `BackAlias run(String args)` | Press or release `options.backKey`; suppresses press on text-input screens only |
