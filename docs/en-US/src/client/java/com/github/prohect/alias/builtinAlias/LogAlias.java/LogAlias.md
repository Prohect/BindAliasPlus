...
Builtin alias that logs a message to the mod's logger (`BindAliasClient.LOGGER`).
...
Key collaborators: extends [BuiltinAliasWithArgs](../../BuiltinAliasWithArgs.java/BuiltinAliasWithArgs.md) directly (not the boolean/double/greedy subclasses), meaning it receives the raw args string as-is. The entire args string is logged at INFO level via `BindAliasClient.LOGGER`.
...
| [BindAliasClient](../../../BindAliasClient.java/BindAliasClient.md) | Owns the LOGGER used here                |
