...
**Behavior:**
- If a container screen (any `HandledScreen`) is currently open: closes it via `close()`.
- If no screen is currently open: opens the player inventory screen by calling `mc.setScreen(new InventoryScreen(mc.player))`.
