...
The message is added via `MinecraftClient.getInstance().inGameHud.getChatHud().addMessage(Text.literal(args))` (Yarn: `Text`; Mojang: `Component`), which displays it in the client-side system message channel. It is **not** sent over the network — the server and other players never see it.
