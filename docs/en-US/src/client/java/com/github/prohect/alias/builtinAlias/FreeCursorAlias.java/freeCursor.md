...
When `true`, `MouseMixin` intercepts `MouseHandler.grabMouse()` and `MouseHandler.releaseMouse()` to skip only the OS-level GLFW cursor grab/release calls. The logical `mouseGrabbed` flag still transitions normally, so game logic that depends on mouse state (hold-to-mine, camera rotation) continues to work.
