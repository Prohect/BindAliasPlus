...
Injected at `HEAD` of `MouseHandler#turnPlayer()`. When `FreeCursorAlias.freeCursor` is `true`, cancels the method entirely so physical mouse delta movements do not rotate the player's camera. This is necessary because freeCursor allows the logical mouse-grab to remain in effect (for mining continuity), which would otherwise re-enable camera turning when the mouse moves over the focused window. Camera control during freeCursor is exclusively via the `yaw`/`pitch`/`setYaw`/`setPitch` aliases.
