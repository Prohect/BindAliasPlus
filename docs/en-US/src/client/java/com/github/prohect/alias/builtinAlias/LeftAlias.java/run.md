...
Simulates a left movement key press or release by calling `KeyMapping.setDown(flag)` and incrementing `clickCount` on press.
...
3. Obtains `Minecraft.getInstance().options.keyLeft`.
4. Calls `leftKey.setDown(flag)` to set the key state.
5. If `flag` is true, increments `leftKey.clickCount` to ensure the game registers the click.

Side effects: modifies `keyLeft.setDown` and `keyLeft.clickCount` on the game's `KeyMapping`.

Callers: dispatched by [UserAlias](../../UserAlias.java/UserAlias.md) when the alias name `"builtinLeft"` matches an `AliasRecord`.
...
| Item                                                                                         | Description                               |
| -------------------------------------------------------------------------------------------- | ----------------------------------------- |
| [LeftAlias](LeftAlias.md)                                                                    | Owning class                              |
...
| [RightAlias.run](../RightAlias.java/run.md)                                                  | Same pattern for `keyRight`               |
