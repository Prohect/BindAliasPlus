...
3. Retrieves `MinecraftClient.getInstance().options.leftKey` and calls `setPressed(flag)` plus `timesPressed++` on press.

Movement is applied each tick by `KeyboardInputMixin`, which reads `leftKey.isDown()`.
