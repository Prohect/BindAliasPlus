...
**Behavior:**
- `+use` (args="1"): Presses the vanilla `keyUse` keybinding (right-click), causing the player to use the held item or interact with the targeted block. Increments `clickCount` for the initial-press event.
- `-use` (args="0"): Releases the `keyUse` keybinding.
