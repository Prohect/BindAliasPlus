# containerSnapshot method (src/client/java/com/github/prohect/mcp/GameStateCollector.java)

## Syntax

```java
static com.github.prohect.mcp.GameStateCollector$ContainerSnapshot containerSnapshot(net.minecraft.client.Minecraft, net.minecraft.client.player.LocalPlayer)
```

## Parameters

| Name | Type | Description |
|------|------|-------------|

## Remarks

## See Also

| Item | Description |
|------|-------------|

*Documented for Commit: [7560eeea61820ae4db1314f3e3132a9576194d5a](https://github.com/Prohect/BindAlias/tree/7560eeea61820ae4db1314f3e3132a9576194d5a)*
