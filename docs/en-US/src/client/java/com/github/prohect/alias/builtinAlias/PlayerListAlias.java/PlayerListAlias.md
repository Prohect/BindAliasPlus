...
**Behavior:**
- `+playerList` (flag=1): Presses the vanilla `playerListKey` keybinding (Tab), making the online-player overlay visible and incrementing `timesPressed` for the first-press tick behavior.
- `-playerList` (flag=0): Releases the `playerListKey` keybinding.
