...
Complex builtin alias for temporarily locking vanilla game keys and custom alias key bindings. Replaces KeyBinding keys with `InputUtil.UNKNOWN_KEY` and tracks locked physical keys in a mixin-checked set.
...
| [getKeyBindingForAction](getKeyBindingForAction.md) | `static KeyBinding getKeyBindingForAction(String)` | Map action type to vanilla KeyBinding |
