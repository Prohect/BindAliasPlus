...
**Behavior:**
- `+openInventory` (flag=1): Opens the player inventory screen by calling `sendOpenInventory()` (server packet) and `McScreenHelper.setScreen()` with a new `InventoryScreen`. Blocked when a text-input screen is open. Blocked when any screen is already open.
