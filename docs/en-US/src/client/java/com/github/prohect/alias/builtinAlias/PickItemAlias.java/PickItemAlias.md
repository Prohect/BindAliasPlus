...
**Behavior:** Simulates pressing the vanilla pick-block keybinding (`pickItemKey`, default: middle mouse button). Sets the keybinding to "down" and increments `timesPressed`, causing the game to call `pickBlockOrEntity()` on the next polling cycle. This selects the hotbar slot matching the targeted block/entity, or moves a matching item from the inventory to the selected slot.
