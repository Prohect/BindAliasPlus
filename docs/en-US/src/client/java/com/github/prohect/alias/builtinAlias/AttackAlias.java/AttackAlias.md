...
The implementation manipulates `MinecraftClient.getInstance().options.attackKey`:

- `setPressed(flag)` — holds or releases the key
- `timesPressed++` — on press, increments the click counter so that vanilla click detection (tap vs. hold) fires
