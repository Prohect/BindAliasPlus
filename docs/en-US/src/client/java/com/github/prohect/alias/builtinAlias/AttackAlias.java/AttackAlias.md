...
Builtin alias that simulates pressing or releasing the attack/destroy key (`keyAttack`).
...
Key collaborators: extends [BuiltinAliasWithBooleanArgs](../../BuiltinAliasWithBooleanArgs.java/BuiltinAliasWithBooleanArgs.md) which provides `parseArgs()` for `"0"`/`"1"` parsing and `flag` field. Uses `Minecraft.getInstance().options.keyAttack` to drive the game's attack key binding.
