...
**Behavior:**
- `+right` (flag=1): Presses the vanilla `keyRight` keybinding (D), making the player strafe right.
- `-right` (flag=0): Releases the `keyRight` keybinding.
...
**Movement injection:** Unlike vanilla, this alias fires `key.setDown(flag)` directly on the `KeyMapping`, which is intercepted by `KeyboardInputMixin.tick()` to inject movement impulses even when the game window doesn't have focus. This is distinct from the vanilla key-press queue, which requires window focus.
