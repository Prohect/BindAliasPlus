...
1. Saves the original key binding (`InputConstants.Key`) for the action.
2. Replaces the `KeyMapping.key` with `InputConstants.UNKNOWN` (GLFW_KEY_UNKNOWN = -1), which GLFW handles gracefully and Minecraft already skips in `releaseAll()` / key polling.
3. Calls `KeyMapping.resetMapping()` to apply the change.
