...
public class SlotAlias extends BuiltinAliasWithArgs<SlotAlias> {
	...
	/**
	 * @param args
	 *            from 1-9, or a variable name that resolves to 1-9
	 */
	...
	public SlotAlias run(String args) {
		...
		/*
		 * KeyBinding hotbarKey = MinecraftClient.getInstance().options.hotbarKeys[i -
		 * 1]; hotbarKey.setPressed(true); hotbarKey.setPressed(false);
		...
		try {
			...
			BindAliasPlusClient.LOGGER.error("{}[Slot]Failed to update selected slot.",
					BindAliasPlusClient.tickPrefix(), e);
		}
		return this;
	}
}
