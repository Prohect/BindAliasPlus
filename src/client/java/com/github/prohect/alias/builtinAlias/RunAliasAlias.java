...
/**
 * Executes a registered alias by name. The argument is the alias name
 * (optionally followed by backslash-args). This is useful in autoload CFG to
 * trigger aliases on world join without pressing a bound key.
 *
 * <p>
...
public class RunAliasAlias extends BuiltinAliasWithGreedyStringArgs<RunAliasAlias> {
	...
	public RunAliasAlias run(String args) {
		...
		if (alias != null) {
			...
			BindAliasPlusClient.LOGGER.warn("{}[runAlias] Unknown alias: {}", BindAliasPlusClient.tickPrefix(),
					aliasName);
		}
		return this;
	}
