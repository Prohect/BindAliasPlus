...
/**
 * Builtin alias to temporarily lock a game key or custom alias, preventing the
 * user's physical key/mouse input from interfering with an alias sequence.
 * <p>
 ...
 * <p>
 * User-facing shortcuts are {@link LockAlias_OnLock}
 * ({@code +lockKey\gameKey:attack}) and {@link LockAlias_Unlock}
 * ({@code -lockKey\gameKey:attack}). To lock a custom UserAlias, use
 * {@code +lockKey\myAlias}.
 */
public class LockAlias extends BuiltinAliasWithArgs<LockAlias> {
	...
	public static final List<String> SUPPORTED_ACTIONS = List.of("gameKey:attack", "gameKey:use", "gameKey:forward",
			"gameKey:back", "gameKey:left", "gameKey:right", "gameKey:jump", "gameKey:sneak", "gameKey:sprint");

	private static final String GAMEKEY_PREFIX = "gameKey:";

	/**
	 * A sentinel key that won't match any physical keyboard/mouse input. Uses
	 * {@link InputUtil#UNKNOWN_KEY} (GLFW_KEY_UNKNOWN = -1) which GLFW handles
	 * gracefully (no "Invalid key" error) and Minecraft already skips in
	 * {@code updatePressedStates()}.
	 */
	private static final InputUtil.Key LOCK_PLACEHOLDER = InputUtil.UNKNOWN_KEY;

	// Maps action type -> original saved bound key
	private static final Map<String, InputUtil.Key> savedBoundKeys = new HashMap<>();

	/** Maps action type -> list of alias name patterns that directly trigger it */
	private static final Map<String, List<String>> ACTION_ALIAS_PATTERNS = new HashMap<>();

	static {
		for (String action : SUPPORTED_ACTIONS) {
			String bare = action.startsWith(GAMEKEY_PREFIX) ? action.substring(GAMEKEY_PREFIX.length()) : action;
			List<String> patterns = new ArrayList<>();
			patterns.add("+" + bare);
			patterns.add("-" + bare);
			patterns.add("builtin" + bare.substring(0, 1).toUpperCase() + bare.substring(1));
			ACTION_ALIAS_PATTERNS.put(bare, patterns);
		}
	}

	/**
	 * Physical keys currently locked. Mixins check this set to also block keys
	 * registered in {@code BINDING_PLUS} during lock.
	 */
	public static final Set<InputUtil.Key> LOCKED_PHYSICAL_KEYS = new HashSet<>();

	/**
	 * Tracks which physical keys are locked for which alias name. Used by the
	 * alias-lock feature so unlock can remove the right keys.
	 */
	private static final Map<String, Set<InputUtil.Key>> LOCKED_ALIAS_KEYS = new HashMap<>();

	// ── alias-name-based locking ─────────────────────────────────────────

	/**
	 * Restore all locked game keys and clear all lock state. Called on server
	 * disconnect to prevent stale key bindings.
	 */
	...
	/**
	 * Lock all physical keys bound to the given alias name. The alias can still be
	 * triggered via {@code runAlias}.
	 */
	static void lockAliasByName(String aliasName) {
		...
		BindAliasPlusClient.BINDING_PLUS.forEach((key, binding) -> {
			if (aliasName.equals(binding.aliasNameOnKeyPressed())
					|| aliasName.equals(binding.aliasNameOnKeyReleased())) {
				keys.add(key);
			}
		});
		if (keys.isEmpty()) {
			BindAliasPlusClient.LOGGER.warn("{}[Lock]No keys bound to alias: {}", BindAliasPlusClient.tickPrefix(),
					aliasName);
			return;
		}
		LOCKED_PHYSICAL_KEYS.addAll(keys);
		LOCKED_ALIAS_KEYS.put(aliasName, keys);
		BindAliasPlusClient.LOGGER.info("{}[Lock]Locked alias '{}' — {} key(s) blocked",
				BindAliasPlusClient.tickPrefix(), aliasName, keys.size());
	}
	...
	static void unlockAliasByName(String aliasName) {
		...
		if (keys == null) {
			BindAliasPlusClient.LOGGER.warn("{}[Lock]Alias not locked: {}", BindAliasPlusClient.tickPrefix(),
					aliasName);
			return;
		}
		LOCKED_PHYSICAL_KEYS.removeAll(keys);
		BindAliasPlusClient.LOGGER.info("{}[Lock]Unlocked alias '{}' — {} key(s) restored",
				BindAliasPlusClient.tickPrefix(), aliasName, keys.size());
	}
	...
	/**
	 * Scan {@code BINDING_PLUS} for keys whose bound aliases (directly or through
	 * UserAlias definitions) target the given action, and add those physical keys
	 * to {@link #LOCKED_PHYSICAL_KEYS}.
	 */
	...
	/**
	 * Remove from {@link #LOCKED_PHYSICAL_KEYS} any keys that were added solely
	 * because of this action type's mod bindings. A key is kept if it is still
	 * needed for another locked action's vanilla boundKey.
	 */
	 ...
	 */
	private static boolean aliasTargetsLockedAction(String aliasName, List<String> patterns) {
		if (aliasName == null || aliasName.isEmpty())
			return false;

		// Direct match: alias name itself is one of the patterns
		if (patterns.contains(aliasName))
			return true;
		...
		if (alias instanceof UserAlias userAlias) {
			...
			for (String token : def.split(Pattern.quote(String.valueOf(Alias.divider4AliasDefinition)))) {
				String[] tokenParts = token.split(Pattern.quote(String.valueOf(Alias.divider4AliasArgs)));
				String aliasPart = tokenParts[0];

				// Handle +lockKey\<action> / -lockKey\<action> — check the concrete action
				if (("+lockKey".equals(aliasPart) || "-lockKey".equals(aliasPart)) && tokenParts.length >= 2) {
					String lockActionName = tokenParts[1];
					for (String pattern : patterns) {
						String barePattern = pattern.startsWith("+") || pattern.startsWith("-")
								? pattern.substring(1)
								: pattern;
						if (barePattern.equals(lockActionName))
							return true;
					}
				}
				if (patterns.contains(aliasPart))
					return true;
			}
		}
		return false;
