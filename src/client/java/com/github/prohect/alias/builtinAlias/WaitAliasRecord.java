...
public class WaitAliasRecord {
	...
	/**
	 * make sure the definition is simply the alias name when
	 * reapplyToGameKeyMapping is true
	 */
	...
	public int tick() {
		if (ticks <= 0) {
			if (reapplyToGameKeyMapping) {
				...
				if (Alias.aliasesWithArgs.get(definition) instanceof BuiltinAliasWithBooleanArgs alias
						&& alias != null) {
