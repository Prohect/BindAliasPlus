...
public class SwapSlotAlias extends BuiltinAliasWithArgs<SwapSlotAlias> {
	...
	public SwapSlotAlias run(String args) {
		...
		try {
			...
			if (cs != null)
				...
				iscr = inInv
						? (creativeInv ? new InventoryScreen(p) : (InventoryScreen) screen)
			...
			try {
				...
				if (im != null) {
					...
					if (s0 != null && s1 != null)
						...
						BindAliasPlusClient.LOGGER.warn("{}[SwitchSlot]slot not found",
								BindAliasPlusClient.tickPrefix());
				} else
					BindAliasPlusClient.LOGGER.warn("{}[SwitchSlot]interactionManager null",
							BindAliasPlusClient.tickPrefix());
			} finally {
				if (iscr != null && !inInv)
					iscr.close();
	...
	private static void click(ClientPlayerInteractionManager im, ScreenHandler menu, Slot s, int btn,
			SlotActionType act, ClientPlayerEntity p) {
