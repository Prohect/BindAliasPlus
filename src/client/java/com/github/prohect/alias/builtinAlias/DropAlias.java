...
public class DropAlias extends BuiltinAliasWithBooleanArgs<DropAlias> {
	 ...
	 * {@link #flag} is {@code true}. After an initial delay (matching the OS
	 * key-repeat gap that vanilla relies on), drives continuous dropping —
	 * {@code slotClicked(…, THROW)} in container screens,
	...
	/**
	 * On cursor re-lock, maintain the KeyMapping state without incrementing
	 * clickCount — otherwise an extra drop would fire.
	 */
	@Override
