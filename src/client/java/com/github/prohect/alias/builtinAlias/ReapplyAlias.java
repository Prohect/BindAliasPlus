 ...
 * <p>
 * Usage: {@code reapply\forward} — calls {@link BuiltinAliasWithBooleanArgs#reapplyToGameKeyMapping()} on the corresponding
 * builtin alias if its {@code flag} is {@code true}.
 * <p>
 * Accepts user-facing action names ({@code forward}, {@code attack}, {@code +forward}, {@code sneak}, etc.) and resolves them
 * to their underlying {@code builtin*} alias.
 * <p>
 * Useful at the end of a UserAlias sequence to re-assert a key that may have been reset by a screen transition.
 */
public class ReapplyAlias extends BuiltinAliasWithArgs<ReapplyAlias> {
    ...
    public static final List<String> SUPPORTED_ACTIONS = List.of("attack", "use", "forward", "back", "left", "right", "jump",
            "sneak", "sprint", "drop", "openInventory", "playerList");
