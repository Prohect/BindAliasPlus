...
/**
 * Feeds the {@link GameChannels#SOUND} channel. Registered as a {@link SoundInstanceListener} on the client
 * {@code SoundManager} — the same hook the vanilla subtitle overlay uses, so exactly the sounds that would show a HUD caption
 * are reported.
 * <p>
 ...
 */
public final class SoundCapture implements SoundInstanceListener {
    ...
    public static void register() {
        MinecraftClient.getInstance().getSoundManager().registerListener(INSTANCE);
    }
    ...
    static String directionOf(ClientPlayerEntity p, double dx, double dy, double dz) {
        ...
        if (horiz >= 0.5) {
            ...
            sb.append("yaw").append(signed(clamp(normalize180(targetYaw - p.getYaw())))).append(' ');
        }
        // MC pitch convention: -90=up, 0=horizon, 90=down
        double targetPitch = -Math.toDegrees(Math.atan2(dy, horiz));
        sb.append("pitch").append(signed(clamp(normalize180(targetPitch - p.getPitch()))));
