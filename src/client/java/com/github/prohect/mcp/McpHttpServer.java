 ...
 * <p>
 * Exposes mod state and alias dispatch so an external MCP bridge can control
 * Minecraft through the mod's alias system. All game-thread access goes through
 * {@link Minecraft#execute(Runnable)} with a 5 s timeout to prevent the HTTP
 * handler from blocking forever.
 * <p>
...
public final class McpHttpServer {
	...
	public static void start() {
		...
		try {
			...
			BindAliasPlusClient.LOGGER.info("{}[MCP] HTTP server started on 127.0.0.1:{}",
					BindAliasPlusClient.tickPrefix(), PORT);
		} catch (Exception e) {
			BindAliasPlusClient.LOGGER.error("{}[MCP] Failed to start HTTP server", BindAliasPlusClient.tickPrefix(),
					e);
		}
	}
	...
	/**
	 * Decode percent-encoded characters (%XX). No '+' → space conversion — the
	 * bridge uses encodeURIComponent which emits spaces as %20, so no special-case
	 * logic is needed.
	 */
	...
	/**
	 * Max length of the compressed container section; beyond this we refuse to
	 * attach it and recommend a screenshot instead.
	 */
	 ...
	 * {@code index} is literally a swapSlot argument - a number (1-41) for player
	 * inventory slots, a {@code "cN"} string for container slots</li>
	 * <li>empty player-inventory slots compress to ranges in the same 1-41
	 * numbering: {@code "1-9 10-36"}</li>
	 * <li>non-inventory slots (chest grid, crafting grid, anvil, ...) compress to
	 * an ASCII map ('#' empty, '$' occupied, ' ' no slot) with the x/y range and
	 * per-cell c-indices alongside</li>
	 * </ul>
	 */
	private static String buildContainerJson(ScreenHandler menu) {
		...
		if (out.length() > CONTAINER_JSON_MAX) {
			return ("{\"menu\":" + jsonEscape(menu.getClass().getName())
	...
	static synchronized void handleScreenshot(HttpExchange exchange) throws IOException {
		try {
			...
			onMainThread(() -> {
				MinecraftClient mc = MinecraftClient.getInstance();
				net.minecraft.client.util.ScreenshotRecorder.saveScreenshot(mc.runDirectory, mc.getFramebuffer(),
						msg -> {
						});
				return null;
			});
			...
			String json = ("{\"path\":" + jsonEscape(path) + ",\"name\":" + jsonEscape(name) + ",\"base64\":"
					+ jsonEscape(b64) + "}");
			sendJson(exchange, 200, json);
		} catch (Exception e) {
			sendJson(exchange, 500, "{\"error\":" + jsonEscape(e.getMessage()) + "}");
	...
	/**
	 * POST /runAlias?def=… — execute a chain of aliases (space-separated, \ for
	 * args).
	 */
	...
	/**
	 * POST /defineAlias?name=…&def=… — define an alias via the real command
	 * pipeline and capture feedback.
	 */
