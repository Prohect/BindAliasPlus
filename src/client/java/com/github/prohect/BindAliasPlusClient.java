...
public class BindAliasPlusClient implements ClientModInitializer {
	...
	public static final Map<InputConstants.Key, KeyBindingPlus> BINDING_PLUS = new HashMap<>();

	public static final Logger LOGGER = LoggerFactory.getLogger("bind-alias-plus");

	/**
	 * Tick counter — incremented every client tick via
	 * {@link ClientTickEvents#START_CLIENT_TICK}.
	 */
	public static long currentTick = 0;
	/**
	 * Tick at which the last {@link ClientPlayConnectionEvents#JOIN} fired, -1 if
	 * never joined.
	 */
	...
	public void onInitializeClient() {
										...
										case 1 -> {
											if (!silentMode) {
												context.getSource().sendFeedback(
														Component.literal("Alias " + name + " = " + definition));
											}
											yield 1;
										}
										case 2, 3 -> {
											if (!silentMode) {
												context.getSource().sendFeedback(
														Component.literal("Can't replace builtinAlias " + name));
											}
											yield 0;
										}
										default -> 0;
						...
						.then(argument("aliasName", StringArgumentType.word()).suggests((context, builder) -> {
							...
							return switch (commandBindByAliasNameExecute(keyName, aliasName)) {
								case 1 -> {
									if (!silentMode) {
										context.getSource().sendFeedback(
												Component.literal("§aBound key " + keyName + " to alias " + aliasName));
									}
									yield 1;
								}
								case 2, 3 -> {
									if (!silentMode) {
										context.getSource().sendFeedback(
												Component.literal("§cAlias " + aliasName + " does not exist!"));
									}
									yield 0;
								}
								case 4 -> {
									if (!silentMode) {
										context.getSource()
												.sendFeedback(Component.literal("§cUnknown key: " + keyName));
									}
									yield 0;
								}
								default -> 0;
								...
								.executes(context -> {
									...
									return switch (commandBindExecute(keyName, definition)) {
										...
										case 3 -> {
											if (!silentMode) {
												context.getSource().sendFeedback(
														Component.literal("bind " + keyName + " = " + definition));
											}
											yield 0;
										}
										default -> 0;
		...
		ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> dispatcher
				.register(literal("unbind").then(argument("key", StringArgumentType.word()).executes(context -> {
					...
					return switch (commandUnbindExecute(keyName)) {
						case 0 -> {
							if (!silentMode) {
								context.getSource().sendFeedback(Component.literal("§cUnknown key: " + keyName));
							}
							yield 0;
						}
						case 1 -> {
							if (!silentMode) {
								context.getSource().sendFeedback(Component.literal("§cUnbind key: " + keyName));
							}
							yield 1;
						}
						default -> 0;
		...
		ClientCommandRegistrationCallback.EVENT.register(
				(dispatcher, registryAccess) -> dispatcher.register(literal("unloadCFGAliases").executes(context -> {
					...
					if (!silentMode) {
						context.getSource().sendFeedback(Component.literal("§aUnloaded all autoloaded aliases"));
					}
					return 1;
				})));
		...
		ClientCommandRegistrationCallback.EVENT.register(
				(dispatcher, registryAccess) -> dispatcher.register(literal("unloadCFGBinds").executes(context -> {
					...
					if (!silentMode) {
						context.getSource().sendFeedback(Component.literal("§aUnloaded all autoloaded keybindings"));
					}
					return 1;
				})));
		...
		ClientCommandRegistrationCallback.EVENT.register(
				(dispatcher, registryAccess) -> dispatcher.register(literal("unloadCFGVars").executes(context -> {
					...
					if (!silentMode) {
						context.getSource().sendFeedback(Component.literal("§aUnloaded all autoloaded variables"));
					}
					return 1;
				})));
		...
		ClientCommandRegistrationCallback.EVENT
				.register((dispatcher, registryAccess) -> dispatcher.register(literal("runAlias")
						.then(argument("aliasName", StringArgumentType.greedyString()).suggests((context, builder) -> {
							...
							String input = StringArgumentType.getString(context, "aliasName");
		...
		if (VarAlias.VARIABLES.containsKey(varName)) {
			...
			if (!silentMode) {
				Minecraft.getInstance().player
						.sendSystemMessage(Component.literal("§cFailed to set variable '" + varName + "'"));
			}
			return 0;
		}
	}
	...
	private int commandBindByAliasNameExecute(String keyName, String aliasName, boolean fromAutoload) {
		...
		/*
		 * t -> +aliasName or it doesn't contain +- and would be triggered when pressing
		 * down as default
		 */
		boolean flag = true;
		...
		BINDING_PLUS.put(key,
				flag
						? new KeyBindingPlus(aliasNameFinal, aliasNameFinalExtra, fromAutoload)
	...
	private static CompletableFuture<Suggestions> getSuggestions4aliasDefinitionCompletableFuture(
		...
		Alias.aliasesWithoutArgs.keySet().forEach(alias -> {
			if (alias.startsWith(currentToken))
				finalBuilder.suggest(alias, Component.literal("alias without args"));
		});
		Alias.aliasesWithArgs.keySet().forEach(alias -> {
			if (alias.startsWith(currentToken))
				finalBuilder.suggest(alias, Component.literal("alias with args"));
		});
