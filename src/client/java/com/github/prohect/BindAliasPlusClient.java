...
public class BindAliasPlusClient implements ClientModInitializer {
	...
	/*
	 * put your elytra in slot 10 ( the first slot of the first row of your
	 * inventory, and firework in slot 19 ( the first slot of the second row of your
	 * inventory, no need to take place of your hotbars, then you can do this
	 */
		...
		if (VarAlias.VARIABLES.containsKey(varName)) {
			if (!silentMode) {
				MinecraftClient.getInstance().player.sendMessage(
						Text.literal("Variable '" + varName + "' set to " + VarAlias.VARIABLES.get(varName)), false);
			}
			return 1;
		} else {
			if (!silentMode) {
