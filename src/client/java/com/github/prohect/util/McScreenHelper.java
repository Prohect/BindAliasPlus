 ...
 * <p>
 * In 26.1.x: {@code Minecraft.screen} (field) and {@code Minecraft.setScreen(Screen)} (method). In 26.2+: both were moved into
 * the {@code Gui} class, accessed via {@code client.gui.screen()} and {@code client.gui.setScreen(Screen)}.
 * <p>
 * Detection is based on whether the {@code Gui} class (the type of {@code Minecraft.gui}) exposes a {@code screen()} method.
 */
public final class McScreenHelper {
    ...
    static {
        ...
        if (!hasGuiScreen) {
            ...
            try {
                setScreenM = Minecraft.class.getMethod("setScreen", Screen.class);
            } catch (Exception ignored) {
            }
        }
