...
scripts/post-checkout          # post-checkout hook: regenerates IDE config on branch switch
```
...
> (generated by the post-checkout hook). See [CLAUDE.md](CLAUDE.md) for usage.
...
The hook script at [`scripts/post-checkout`](scripts/post-checkout) regenerates
IDE config when switching branches:
